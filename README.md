...
اگر اتصال مستقیم `۴۲۹` گرفت، ربات لیست پروکسی‌های رایگان ProxyScrape را می‌گیرد (آدرسش در `config.yaml` قابل عوض‌کردن است) و آی‌پی‌به‌آی‌پی می‌چرخد تا یک پروکسی جواب بدهد؛ تعداد آزمایش‌ها از variable به نام `PROXY_TRIES` خوانده می‌شود (پیش‌فرض ۸). پروکسی‌های رایگان اکثراً مرده‌اند، پس انتظار نداشته باش که این لایه نجات بدهد؛ ارزش اصلی‌اش این است که جای انتظار کشیدن، سریع رد می‌شود.

برای این‌که هیچ اجرایی بی‌خودی ۱۵ دقیقه‌ی job را پر نکند، دو سقف زمانی داخل کد هست: هر مسیر (مستقیم یا یک پروکسی) حداکثر `ROUTE_BUDGET` ثانیه وقت دارد (پیش‌فرض ۱۵۰) و کل اجرا `RUN_BUDGET` ثانیه (پیش‌فرض ۶۰۰، از variable هم قابل تغییر است). بعد از آن سراغ مسیر بعدی می‌رود و اگر همه بند آمده باشند workflow را خاموش می‌کند.
